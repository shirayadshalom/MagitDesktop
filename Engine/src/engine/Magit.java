...
public class Magit {
   ...
   public Magit(MagitController magitController){
    ...
    public Boolean checkXml (String path) throws JAXBException, Exception{
        ...
        File file = new File(xmlRepo.getLocation() + "\\" + ".magit");
        return (!(file.exists()));
    }
    ...
    public void LoadFromXML (String path) throws JAXBException, Exception{
        ...
        createobjects(xmlRepo);
        activerepo = new Repository(xmlRepo.getLocation());
        activerepo.getRootFolder().deploy(xmlRepo.getLocation());
        objectsXml.clear();
    }
