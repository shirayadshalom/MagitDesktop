...
public class Xml {
    ...
    public static void createobjects(MagitRepository xmlRepo) throws IOException, ClassNotFoundException {

        File f = new File(xmlRepo.getLocation());
        if ((f.exists())) {
        ...
        createFolders(xmlRepo.getName(), xmlRepo.getLocation());
        for (MagitSingleFolder folder : xmlRepo.getMagitFolders().getMagitSingleFolder()) {
            loadFolderFromXml(folder, xmlRepo);
        }
        for (MagitSingleCommit commit : xmlRepo.getMagitCommits().getMagitSingleCommit()) {
            loadCommitFromXml(commit, xmlRepo);
        }
        writeToFile(xmlRepo.getMagitBranches().getHead(), xmlRepo.getLocation() + "\\.magit\\branches" + "\\head");
        for (MagitSingleBranch branch : xmlRepo.getMagitBranches().getMagitSingleBranch()) {
            addBranchFromXml(branch, xmlRepo);
        }

        if (xmlRepo.getMagitRemoteReference() != null && xmlRepo.getMagitRemoteReference().getName() != null)
            try (Writer out = new BufferedWriter(
                    new OutputStreamWriter(new FileOutputStream(xmlRepo.getLocation() + "\\.magit\\remoteRepoPath"), "UTF-8"))) {
                out.write(xmlRepo.getMagitRemoteReference().getLocation());
            }
    }

    public static void addBranchFromXml(MagitSingleBranch branch, MagitRepository xmlRepo) throws IOException {
        Commit head = loadCommitFromXml(getCommit(branch.getPointedCommit().getId(), xmlRepo), xmlRepo);
        Branch branch1;
        if (branch.isIsRemote())
            branch1 = new Branch(branch.getName(), null, head, xmlRepo.getMagitRemoteReference().getName(), xmlRepo.getLocation());
        else{
            branch1 = new Branch(branch.getName(), null, head, xmlRepo.getLocation());
        }
        if (branch.isTracking())
            branch1.setRTB();
    ...
    public static Commit loadCommitFromXml(MagitSingleCommit commit, MagitRepository xmlRepo) throws IOException {
        Folder rootFolder = loadFolderFromXml(getFolder(commit.getRootFolder().getId(), xmlRepo), xmlRepo);
        Commit com;
        if ((commit.getPrecedingCommits() == null)||(commit.getPrecedingCommits().getPrecedingCommit().size()==0))
            ...
            if(commit.getPrecedingCommits().getPrecedingCommit().size() == 2){
                Commit prevCom = loadCommitFromXml(getCommit(commit.getPrecedingCommits().getPrecedingCommit().get(0).getId(), xmlRepo), xmlRepo);
                Commit prev2Com = loadCommitFromXml(getCommit(commit.getPrecedingCommits().getPrecedingCommit().get(1).getId(), xmlRepo), xmlRepo);
        ...
        addObjectFromXml(com, xmlRepo.getLocation());
        return com;
    }

    public static Blob loadBlobFromXml(MagitBlob blob, MagitRepository xmlRepo) throws IOException {
        String path = findPath(blob, xmlRepo);
        Blob b = new Blob(blob.getLastUpdater(), blob.getName(), xmlRepo.getLocation() + path + "\\" + blob.getName(), blob.getContent(), blob.getLastUpdateDate());
        addObjectFromXml(b,xmlRepo.getLocation());
        return b;
    }

    public static Folder loadFolderFromXml(MagitSingleFolder folder, MagitRepository xmlRepo) throws IOException {
        String path = findPath(folder, xmlRepo);
        Folder f;
        if (folder.isIsRoot())
            f = new Folder(folder.getLastUpdater(), "root", xmlRepo.getLocation(), folder.getLastUpdateDate());
        else
            f = new Folder(folder.getLastUpdater(), folder.getName(), xmlRepo.getLocation() + path + "\\" + folder.getName(), folder.getLastUpdateDate());
        addFolderXml(f, folder, xmlRepo);
        addObjectFromXml(f, xmlRepo.getLocation());
        return f;
    }

    public static void addFolderXml(Folder folder, MagitSingleFolder xmlFolder, MagitRepository xmlRepo) throws IOException {
        for (Item item : xmlFolder.getItems().getItem()) {
            if (item.getType().equals("blob")) {
                Blob b = loadBlobFromXml(getBlob(item.getId(), xmlRepo), xmlRepo);
                folder.getMap().put(b.path, b);
            } else {
                Folder f = loadFolderFromXml(getFolder(item.getId(), xmlRepo), xmlRepo);
